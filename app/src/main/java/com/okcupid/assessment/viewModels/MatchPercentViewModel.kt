...
/**
 * View model used on Match Percent page
 */
class MatchPercentViewModel(repository: OkcupidRepository): BaseViewModel(repository) {

    companion object {
        private val TAG = MatchPercentViewModel::class.java.simpleName
    }
    ...
    override fun fetchData(loadFromCache: Boolean) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                if (loadFromCache) {
                    petList.value = getFavoriteItems(cachePetList, favoriteItems)
                } else {
                    when (val result = repository.getPetList()) {
                        is DataResult.Success -> {
                            loading.value = LoadResult.SUCCESS
                            cachePetList = result.data.cloneList()
                            petList.value = getFavoriteItems(result.data, favoriteItems)
                        }
    ...
    private fun getFavoriteItems(list: List<PetItem>, favItems: List<PetMatch>): List<PetItem> {
        val clonedList = list.cloneList()
        clonedList.forEach { item ->
            item.liked = favItems.find { it.userId == item.userId } != null
        }

        return clonedList.filter { item -> item.liked }.sortedByDescending { it.match }
    }
